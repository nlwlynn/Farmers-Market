...
namespace Controller
{
    ...
    public class DogScript : MonoBehaviour
    {
        ...
        private enum DogState { Idle, Wander, Return, FaceForward }
        private DogState currentState = DogState.Wander;
        ...
        private void Awake()
        {
            GameObject spawnPoint = GameObject.Find("Dog-Spawn");
        ...
        private void HandleAI(float deltaTime)
        {
            ...
            switch (currentState)
            {
                case DogState.Wander:
                    bool needNewTarget = Vector3.Distance(transform.position, m_Target) < 1.0f || 
                                         m_Target == Vector3.zero ||
                                         UnityEngine.Random.value < 0.01f;

                    if (needNewTarget)
                    {
                        float randomDistance = UnityEngine.Random.Range(3f, 8f);
                        var randomDir = UnityEngine.Random.insideUnitCircle.normalized;
                        Vector3 targetPosition = transform.position + new Vector3(randomDir.x * randomDistance, 0, randomDir.y * randomDistance);

                        Vector3 directionToSpawn = targetPosition - spawnPosition;
                        if (directionToSpawn.magnitude > 20f) 
                        {
                            Vector3 towardSpawn = (spawnPosition - transform.position).normalized;
                            targetPosition = transform.position + new Vector3(towardSpawn.x * randomDistance, 0, towardSpawn.z * randomDistance);
                        }
                    ...
                    toTarget.y = 0; 
                    Vector2 wanderDir = new Vector2(toTarget.x, toTarget.z).normalized;

                    SetInput(wanderDir, m_Target, false, false);

                    if (stateTimer >= wanderDuration) 
                    {
                        stateTimer = 0f;
                        currentState = DogState.Return;
                        m_Target = spawnPosition;
                    }
                    break;
                ...
                case DogState.Return:
                    ...
                    {
                        currentState = DogState.FaceForward; 
                        stateTimer = 0f;
                    }
                    else
                    {
                        Vector2 moveDir = new Vector2(toSpawn.x, toSpawn.z).normalized;
                        SetInput(moveDir, spawnPosition, false, false);
                    }
                    break;
